...
def test_base_html(client: FlaskClient,
    ...
    auth.login()
    second_response = client.get(curr_url)
    assert second_response.get_data(as_text=True) != first_response.get_data(
        as_text=True)
