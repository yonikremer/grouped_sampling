...
def get_end_of_text_id(tokenizer: PreTrainedTokenizer, config: AutoConfig):
    ...
    if hasattr(tokenizer,
               "eos_token_ids") and tokenizer.eos_token_ids is not None:
    ...
    if (hasattr(tokenizer, "_eos_token_type_id")
            and tokenizer.eos_token_type_id is not None):
...
def get_tokenizer_name(model_name: str, ) -> str:
    ...
    if model_name.startswith("Aleksandar1932/gpt2") or model_name.startswith(
            "Azaghast/GPT2"):
        return "gpt2"
