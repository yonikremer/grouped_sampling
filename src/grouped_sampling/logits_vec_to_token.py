...
import torch
from torch import FloatTensor, Tensor, argmax, exp, long, multinomial
...
class LogitVectorToTokenPipeLine:
    ...
    def single_logit_vector_to_token(self, input_ids: Tensor,
                                     logits: FloatTensor, **kwargs) -> long:
        """
        ...
        wrapped_logits = self.logit_wrapper(input_ids=input_ids,
                                            scores=logits,
                                            **kwargs)
        if self.do_sample:
    ...
    def logit_matrix_to_tokens(self, input_ids: Tensor, logit_vectors: Tensor,
                               **kwargs) -> Tensor:
        """
        ...
        def logit_vector_to_token(vector) -> long:
            return self.single_logit_vector_to_token(input_ids, vector,
                                                     **kwargs)

        tokens = [
            logit_vector_to_token(logit_vector)
    ...
    def batch_to_tokens(
        ...
        if not all(logit_matrix.dtype in (torch.float, torch.float16)
        ...
        if any(logit_matrix.dim() != 2 or min(logit_matrix.shape) == 0
               for logit_matrix in batch):
            raise ValueError(
