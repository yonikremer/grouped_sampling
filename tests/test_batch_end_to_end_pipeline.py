...
def validate_logits(pipeleine: BatchEndToEndSingleSequencePipeLine, logits: List[Tensor], output_length: int) -> None:
    ...
    if not all(logit.device == pipeleine.device for logit in logits):
        raise ValueError(f"logits should be on device {pipeleine.device}, got {logits[0].device} for some logit")
    if not all(logit.dtype == float32 for logit in logits):
        raise ValueError(f"logits should have dtype {float32}, got {logits[0].dtype} for some logit")
    if not all(logit.dim() == 2 for logit in logits):
        raise ValueError(f"logits should be 2D tensors, got {logits[0].dim()}D tensor for some logit")
...
def validate_padded_tokens(pipeline: BatchEndToEndSingleSequencePipeLine, padded_tokens: Tensor) -> None:
    ...
    if padded_tokens.shape[1] > pipeline.max_total_len:
        raise ValueError(f"tokens should have at most {pipeline.max_total_len} columns, got {padded_tokens.shape[1]}")
    ...
    if not all(0 <= token < pipeline.tokenizer.vocab_size for token in padded_tokens.flatten()):
        raise ValueError("tokens should be valid token ids")
    if padded_tokens.device != pipeline.device:
        raise ValueError(f"tokens should be on device {pipeline.device}, got {padded_tokens.device}")
    if any(padded_tokens[i, -1] == padded_tokens[i, 0] for i in range(padded_tokens.shape[0])):
...
def validate_output_tokens(pipeline: BatchEndToEndSingleSequencePipeLine, output_tokens: List[Tensor],
    ...
    if not all(tokens.dim() == 1 for tokens in output_tokens):
        raise ValueError(f"output_tokens should be 1D tensors, got {output_tokens[0].dim()}D tensor for some tokens")
    if not all(0 <= token < pipeline.tokenizer.vocab_size for sequence in output_tokens for token in sequence):
...
class TestBatchEndToEndSingleSequencePipeLine:
    ...
    def test_init_8bits_model(self):
        import bitsandbytes
        assert bitsandbytes.COMPILED_WITH_CUDA, "bitsandbytes was not compiled with CUDA"
        pipeline = BatchEndToEndSingleSequencePipeLine('fxmarty/tiny-llama-fast-tokenizer')
        prompts = ['Hello', 'How are you?']
