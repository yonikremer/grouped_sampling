...
def validate_logits(
    ...
    if not all(logit.shape[1] == pipeleine.tokenizer.vocab_size
               for logit in logits):
        raise ValueError(
            f"logits should have {pipeleine.tokenizer.vocab_size} columns")


def validate_padded_tokens(pipeline: BatchEndToEndSingleSequencePipeLine,
...
def validate_output_tokens(
    ...
    if not all(0 <= token < pipeline.tokenizer.vocab_size
               for sequence in output_tokens for token in sequence):
        raise ValueError("output_tokens should be valid token ids")
    if not all(sequence.shape[0] <= output_length
               for sequence in output_tokens):
        raise ValueError(
            "output_tokens should have at most output_length tokens")
...
class TestBatchEndToEndSingleSequencePipeLine:
    ...
    def test_empty_prompts(self):
        ...
        output_length = 5
        expected_output = []
        assert pipeline.genearte_batch(prompts,
                                       output_length) == expected_output
    ...
    def validate_pipeline(pipeline):
        ...
        assert isinstance(
            pipeline.tokenizer, PreTrainedTokenizer
        ) or isinstance(
        ...
        assert isinstance(pipeline.model,
                          OptimizedModule), "model is not OptimizedModule"
